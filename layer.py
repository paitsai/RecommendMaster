...
class userEmbeddingLayer(nn.Module):
    def __init__(self):
        ...
        # 线性层定义
        self.userid_fc = nn.Linear(1, self.hidden_embedding_size).to(self.device)
        self.gender_fc = nn.Linear(1, int(self.hidden_embedding_size / 2)).to(self.device)
        self.age_fc = nn.Linear(1, self.hidden_embedding_size).to(self.device)
        self.occupation_fc = nn.Linear(1, self.hidden_embedding_size).to(self.device)
        self.ll1 = nn.Linear(56, 128).to(self.device)
        self.softmax1 = nn.ReLU()
        self.ll2 = nn.Linear(128, 32).to(self.device)
        self.softmax2 = nn.Softmax(dim=0)

    def forward(self, record: str):
        # 对数据进行处理
        info_list = record.split("::")
        self.userid = torch.tensor(int(info_list[0])).unsqueeze(0).float().to(self.device)  # 转换为 1D 张量并转为 float
        self.gender = torch.tensor(1 if info_list[1] == 'M' else 0).unsqueeze(0).float().to(self.device)
        self.age = torch.tensor(int(info_list[2])).unsqueeze(0).float().to(self.device)
        self.occupation = torch.tensor(int(info_list[3])).unsqueeze(0).float().to(self.device)
...
class moviesEmbeddingLayer(nn.Module):
    def __init__(self):
        ...
        # 线性层和 BERT 模型初始化
        self.movieid_fc = nn.Linear(1, self.hidden_embedding_size).to(self.device)
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        self.bert = BertModel.from_pretrained("bert-base-uncased").to(self.device)
        self.ll1 = nn.Linear(self.hidden_embedding_size + 768 * 2, 128).to(self.device)  # Bert 输出的维度为 768
        self.softmax1 = nn.ReLU()
        self.ll2 = nn.Linear(128, 32).to(self.device)
        self.softmax2 = nn.Softmax(dim=0)
    ...
    def forward(self, record: str):
        # 处理电影数据
        info_list = record.split("::")
        self.movieid = torch.tensor(int(info_list[0])).unsqueeze(0).float().to(self.device)  # 转换为 1D 张量并转为 float
        ...
        # 使用 BERT 进行文本编码
        self.title_token = self.tokenizer(self.title, return_tensors='pt', padding=True, truncation=True).to(self.device)
        self.title_embedding = self.bert(**self.title_token).pooler_output  # 使用 pooler_output
        self.genres_token = self.tokenizer(self.genres, return_tensors='pt', padding=True, truncation=True).to(self.device)
...
# 示例：如何创建模型实例
# user_layer = userEmbeddingLayer().to(user_layer.device)
# movie_layer = moviesEmbeddingLayer().to(movie_layer.device)
